...
// spazio del 'main program': tutte le istruzioni di disegno.
    ...
    $lightGreen = imageColorAllocate($im,150,237,146);

    // delimito l'area totale della tela.
    imageRectangle($im, 0, 0, WIDTH-1, HEIGHT-1, $black);
   
    // delimito l'area utile del grafico.
    imageRectangle($im, MLEFT, MTOP, WIDTH-MRIGHT, HEIGHT-MBOTTOM, $black);
    ...
    // preparo un vettore con N elmenti da graficare.
    $valori = Array();
    // recupero i valori da graficare da una tabella presente nel database
    $db = new mysqli("localhost","root","","scuola2021");
        $sql = "SELECT valore FROM meteo_temperature";
    ...
    for($i=0; $i<count($valori); $i++){
        imageStringUp($im, 3, MLEFT+$x+($xoffset/4), HEIGHT-MBOTTOM+35, $valori[$i], $red);
        $x += $xoffset;
    }
    ...
    // disegno le barre sul grafico proporzionate al valore massimo da rappresentare.
    $x = $xoffset/2;
    for($i=0; $i<count($valori); $i++){
        //imageFilledEllipse($im, getGX($x), getGY($valori[$i], $vmax), 10, 10, $blue);
        imageFilledRectangle($im,getGX($x),HEIGHT-MBOTTOM,getGX($x+$xoffset), getGY($valori[$i], $vmax), $lightGreen);
        imageRectangle($im,getGX($x),HEIGHT-MBOTTOM,getGX($x+$xoffset), getGY($valori[$i], $vmax), $black);
        $x = $x + $xoffset;
    }
